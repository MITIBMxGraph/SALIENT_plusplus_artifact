...
def dataset_feasibility(free_gb):
    if args.max_num_nodes >= 16:
        multiplier = 5
    elif args.max_num_nodes >= 8:
        multiplier = 4
    elif args.max_num_nodes >= 4:
        multiplier = 3
    elif args.max_num_nodes >= 2:
        multiplier = 2
    else:
        print("[Error] you specified --max_num_nodes N where N < 2. These experiments are designed for testing a distributed training system among multiple nodes.")
        exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_num_nodes", help="Specify the maximum number of nodes (GPUs) you will use for experiments. This determines which preprocessed partitioned datasets are downloaded --- e.g., 8-way partitioned datasets will not be downloaded if you specify a value here of less than 8.", type=int, required=True)
    try:
